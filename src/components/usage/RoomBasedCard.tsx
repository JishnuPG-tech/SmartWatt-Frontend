...
export function RoomBasedCard({
  ...
  const addRoom = () => {
    const newId = (
      Math.max(...rooms.map((r) => parseInt(r.id))) + 1
    ).toString();
    setRooms([
      ...rooms,
      { id: newId, name: `Room ${rooms.length + 1}`, hours: 2 },
    ]);
  };
  ...
  return (
    <div className="p-6 bg-[#1a202c] border border-[#4a5568] rounded-xl mb-6 shadow-sm">
      <h4 className="text-lg font-medium text-[#e2e8f0] mb-6 flex items-center gap-3">
        ...
        <span>{title}</span>
        {count > 1 && (
          <span className="bg-slate-700 text-slate-300 text-xs px-2 py-1 rounded ml-2">
            {count} units
          </span>
        )}
      </h4>
      ...
      {fields && fields.length > 0 && (
        <div className="mb-6 space-y-4">
          {fields
            .filter((field) => !field.key.includes("num_"))
            .map((field) => (
              <div key={field.key}>
                <label className="text-[#e2e8f0] mb-2 block text-sm">
                  {field.label}
                </label>
                <select
      ...
      {mode === "simple" ? (
        ...
        <div className="animate-in fade-in slide-in-from-left-4 duration-300">
          <div className="mb-6">
            <label className="text-[#e2e8f0] font-medium mb-4 block">
              ...
              {presets.map((preset) => (
                ...
                >
                  <div className="flex flex-col gap-1">
                    <span
                      className={`text-sm font-medium ${currentHours === preset.val ? "text-blue-300" : "text-slate-200"}`}
                    >
                      {preset.label.split("(")[0]}
                    </span>
                    <span
                      className={`text-xs ${currentHours === preset.val ? "text-blue-200/70" : "text-slate-500"}`}
                    >
        ...
        <div className="animate-in fade-in slide-in-from-right-4 duration-300">
          <div className="flex justify-between items-center mb-4">
            <label className="text-[#e2e8f0] font-medium">
              Room-wise Breakdown
            </label>
            <span className="text-blue-400 font-bold bg-blue-900/20 px-3 py-1 rounded-full text-sm border border-blue-500/30">
      ...
      {alert && (
        <div
          className={`mt-6 p-4 rounded-lg border-l-4 flex items-start gap-3 ${
            alert.type === "warning"
