      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          fields.map((field) => (
            <div key={field.key}>
              ...
              <div className="relative">
                <select
                  value={
                    values[field.key]?.toString() ?? field.options[0].value
                  }
        ...
        {usagePatterns && usagePatterns.length > 0 && (
          <div className="md:col-span-2">
            <label className="text-[#e2e8f0] mb-3 block text-sm font-medium">
              Usage Pattern
            </label>
              ...
              {usagePatterns.map((opt) => (
                ...
                >
                  <div className="flex flex-col gap-1">
                    <span
                      className={`text-sm font-medium ${selectedPattern === opt.value ? "text-blue-300" : "text-slate-200"}`}
                    >
                      {opt.label?.includes("(")
                    ...
                    {opt.label?.includes("(") && (
                      <span
                        className={`text-xs ${selectedPattern === opt.value ? "text-blue-200/70" : "text-slate-500"}`}
                      >
                        {opt.label.match(/\((.*?)\)/)?.[1]}
                        {opt.label.includes(") - ")
                          ? ` • ${opt.label.split(") - ")[1]}`
