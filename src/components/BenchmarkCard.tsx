        ...
        <div className="flex flex-col md:flex-row gap-8 items-center">
          ...
          <div className="flex-1 text-center md:text-left">
            <p className="text-slate-400 text-xs uppercase tracking-wider mb-2">
              Vs. Average Kerala Home ({baseline} kWh)
            </p>
            <div className="flex items-baseline gap-3 justify-center md:justify-start">
              <span
                className={`text-4xl font-bold ${isEfficient ? "text-green-400 drop-shadow-[0_0_10px_rgba(74,222,128,0.3)]" : "text-orange-400 drop-shadow-[0_0_10px_rgba(251,146,60,0.3)]"}`}
              >
                {isEfficient
                  ? `${Math.round(efficiency)}% Better`
                  : `${Math.round(Math.abs(efficiency))}% Higher`}
              </span>
            ...
            <div
              className={`inline-flex items-center gap-2 mt-3 px-3 py-1 rounded-full text-xs font-medium border ${isEfficient ? "bg-green-500/10 border-green-500/20 text-green-300" : "bg-orange-500/10 border-orange-500/20 text-orange-300"}`}
            >
              {isEfficient ? (
          ...
          <div className="flex-1 w-full bg-[#1a202c]/50 p-6 rounded-xl border border-slate-700/50">
            <div className="relative h-4 bg-slate-800 rounded-full overflow-hidden shadow-inner">
              {/* Marker for Avg */}
              <div
                className="absolute top-0 bottom-0 w-0.5 bg-white/70 z-10 shadow-[0_0_5px_rgba(255,255,255,0.8)]"
                style={{ left: `${(baseline / (baseline * 1.5)) * 100}%` }}
              ></div>
