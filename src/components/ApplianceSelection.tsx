  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 animate-in fade-in duration-700">
      ...
      <p className="text-[#e2e8f0] mb-8">
        Select all appliances you have at home
      </p>
      ...
      <div className="section space-y-8">
        {APPLIANCE_CATEGORIES.map((cat) => (
          <div key={cat.title}>
            <h3 className="text-[#e2e8f0] font-medium text-lg mb-4">
              {cat.title}
            </h3>
              ...
              {cat.items.map((item: any) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${selected.includes(item.id) ? "bg-blue-600/20 text-blue-400" : "bg-slate-800 text-slate-500 group-hover:bg-slate-700 group-hover:text-slate-300"}`}
                    >
                      <item.icon
                        className={`w-5 h-5 ${item.color || "text-slate-400"}`}
                      />
                    </div>
                  ...
                  {selected.includes(item.id) && item.quantityConfig && (
                    <div
                      className="mt-3 pt-3 border-t border-slate-700/50 animate-in fade-in"
                    ...
                    >
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="bg-blue-600/20 text-blue-300 px-2 py-0.5 rounded text-xs font-bold">
                          {details[item.quantityConfig.key] ??
                            item.quantityConfig.defaultValue}
                        </span>
                      </div>
                      <input
                        type="range"
