              ...
              return (
                ...
                >
                  <td className="p-4 whitespace-nowrap">
                    {new Date(entry.date).toLocaleDateString()}
                    <span
                      className={`text-xs block ${isSelected ? "text-blue-300" : "text-slate-500"}`}
                    >
                  ...
                  <td className="p-4 text-right font-mono">
                    {Math.round(entry.kwh)}
                  </td>
                  <td
                    className={`p-4 text-right font-mono font-medium ${isSelected ? "text-green-300" : "text-green-400"}`}
                  >
                    ₹{Math.round(entry.bill)}
                  </td>
                  {onDelete && (
                    <td className="p-4 text-right">
                      <button
                        onClick={(e) => {
                          ...
                          if (
                            confirm(
                              "Are you sure you want to delete this specific history entry?",
                            )
                          ) {
                            onDelete(entry);
                          }
