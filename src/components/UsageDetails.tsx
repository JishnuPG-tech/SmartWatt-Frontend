...
export default function UsageDetails({
  ...
  const pages = [
    ...(categories.major.length
      ? [
          {
            id: "major",
            title: "Major Appliances",
            subtitle: "High-impact appliances - typically 60-80% of your bill",
            items: categories.major,
          },
        ]
      : []),
    ...(categories.kitchen.length
      ? [
          {
            id: "kitchen",
            title: "Kitchen Appliances",
            subtitle: "Cooking and preparation",
            items: categories.kitchen,
          },
        ]
      : []),
    ...(categories.lighting.length
      ? [
          {
            id: "lighting",
            title: "Lighting & Fans",
            subtitle: "Lighting and ventilation systems",
            items: categories.lighting,
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            subtitle: "Electronics and other devices",
  ...
  if (totalPages === 0) {
    return (
      <div className="w-full max-w-4xl mx-auto px-4 text-center py-12">
        ...
        <div className="st-alert-info mb-6">
          No appliances selected for detailed analysis.
        </div>
        <button onClick={onNext} className="st-button">
          Calculate Results →
        </button>
      </div>
    );
  ...
  const renderAppliance = (id: string) => {
    ...
    if (formConfig.pumpBased) {
      return (
        <PumpDetailCard
          ...
          icon={
            <displayInfo.icon
              className={`w-5 h-5 ${displayInfo.color || "text-blue-400"}`}
            />
          }
    ...
    if (formConfig.roomBased) {
      ...
      if ("quantityConfig" in displayInfo && displayInfo.quantityConfig) {
        const quantityConfig = displayInfo.quantityConfig as {
          key: string;
          defaultValue?: number;
        };
        count = parseInt(
          (details as Record<string, any>)[quantityConfig.key] ||
            quantityConfig.defaultValue?.toString() ||
            "1",
        );
      } else if (formConfig.fields) {
        const countField = formConfig.fields.find((f) =>
          f.key.includes("num_"),
        );
        if (countField) {
      ...
      return (
        <RoomBasedCard
          ...
          quantityConfig={
            "quantityConfig" in displayInfo
              ? (displayInfo.quantityConfig as
                  | {
                      key: string;
                      label: string;
                      min: number;
    ...
    return (
      <ApplianceDetailCard
        ...
        selectedPattern={
          details[`${dataKey}_pattern`] || formConfig.defaultPattern || ""
        }
        onPatternChange={(val) =>
          handlePatternChange(dataKey, val, formConfig.patterns || [])
        }
        onFieldChange={updateDetail}
        values={details}
        alert={getUsageAlert(dataKey, details[`${dataKey}_hours`])}
    ...
    <div className="w-full max-w-7xl mx-auto px-4 animate-in fade-in duration-700">
      ...
      <div className="section mb-8">
        <div className="flex justify-between text-sm text-[#cbd5e0] mb-2 font-medium">
          <span>
            Step {subStep + 1} of {totalPages + 2}: {currentPage?.title}
          </span>
      ...
      {/* Appliance List */}
      <div className="space-y-6">{currentPage?.items.map(renderAppliance)}</div>
