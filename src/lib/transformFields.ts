...
function transformFieldValues(
  ...
  if (
    transformed.fridge_capacity_liters &&
    typeof transformed.fridge_capacity_liters === "string"
  ) {
  ...
  // WM capacity: "7.0" → 7.0
  if (
    transformed.wm_capacity_kg &&
    typeof transformed.wm_capacity_kg === "string"
  ) {
  ...
  if (
    transformed.water_heater_capacity_liters &&
    typeof transformed.water_heater_capacity_liters === "string"
  ) {
    const match = transformed.water_heater_capacity_liters.match(/(\d+)/);
    transformed.water_heater_capacity_liters = match
      ? parseFloat(match[1])
      : 15;
  }
  ...
  if (
    transformed.fridge_age_years &&
    typeof transformed.fridge_age_years === "string"
  ) {
  ...
  if (
    transformed.water_heater_age &&
    typeof transformed.water_heater_age === "string"
  ) {
  ...
  // TV size: "43" → 43
  if (
    transformed.tv_size_inches &&
    typeof transformed.tv_size_inches === "string"
  ) {
  ...
  // Pump HP: "1.0" → 1.0
  if (
    transformed.water_pump_hp &&
    typeof transformed.water_pump_hp === "string"
  ) {
  ...
  if (transformed.wm_type) {
    ...
    transformed.wm_type =
      typeMap[String(transformed.wm_type)] || transformed.wm_type;
  }
 ...
 */
function deriveFieldsFromPattern(
  appliance: string,
  data: Record<string, unknown>,
): Record<string, unknown> {
  const derived = { ...data };
  ...
  if (
    (appliance === "ac" || appliance === "air_conditioner") &&
    data.ac_pattern
  ) {
  ...
  if (
    (appliance === "wm" || appliance === "washing_machine") &&
    data.wm_pattern
  ) {
 ...
 */
export function transformApplianceData(
  appliance: string,
