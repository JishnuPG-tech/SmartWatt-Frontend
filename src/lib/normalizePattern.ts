...
export function normalizePattern(text: string): NormalizedUsage {
  ...
  // Matches: "hours/day", "hr/day", "hrs/night", "hours)", "hrs)"
    ...
    text.match(/(?:hours?|hrs?)(?:\/(?:day|night))?/i) ||
    text.match(/(?:hours?|hrs?)\)/i)
  ) {
    // Check for "X-Y" range (supports hyphen and en-dash)
    const rangeMatch = text.match(
      /(\d+(?:\.\d+)?)\s*[-–]\s*(\d+(?:\.\d+)?)\s*(?:hours?|hrs?)/i,
    );
    if (rangeMatch) {
  ...
  // 4. Handle "X-Y min" or "X min" (Daily)
    ...
    text.match(/(?:min|minutes?)\/day/i) ||
    text.match(/(?:min|minutes?)\)/i)
  ) {
  ...
  if (text.includes("24x7")) {
    ...
    text.toLowerCase().includes("always on") ||
    text.toLowerCase().includes("24 hours")
  ) {
    if (max === 0) {
