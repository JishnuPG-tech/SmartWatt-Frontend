export const generatePDF = async (
  ...
  try {
    ...
    const summaryData = [
      [
        `Household Size: ${household.num_people} People`,
        `Season: ${household.season.toUpperCase()} `,
      ],
      [
        `House Type: ${household.house_type.toUpperCase()} `,
        `Bi - Monthly Units: ${household.kwh} kWh`,
      ],
      [
        `Estimated Bill: Rs.${Math.floor(billDetails.total)} `,
        `Avg Cost / Unit: Rs.${avgCost.toFixed(2)} `,
      ],
    ];
